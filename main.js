...
function render() {
  ...
  bindSettings()
    ...
    app.innerHTML = `<div class="app-shell"><section class="manage-card"><h2>Could not load</h2><p class="muted tight">${escapeHtml(error && error.message ? error.message : 'Unknown error')}</p><button class="primary-btn full" data-action="reload-app">Reload</button></section></div>`
  }
}
...
if ('serviceWorker' in navigator) {
  // The old sw.js served a stale/broken shell on some phones and kept the
  // installed app stuck on "Loading Daily Report". This build ships no
  // service worker, so remove any previously registered one plus its caches.
